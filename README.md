...
Привет! Тут я представляю программу, решающую задачу по составлении статистики о пролонгации менеджеров. Приятного просмотра!😉
...
`re` - пригодится для распознения слов по маске
...
Если мы `x` встречаем как `стоп` или `end`, то мы возвращаем `стоп`. Аналогично для `ноль`. Это поможет нам привести некоторые значения в удобный и гарантированный для нас формат, что уменьшает вероятность возникновения ошибки. Ну а все остальные значения — это числа. В них мы заменяем запятые на точки и удаляем все пробелы внутри. В случае работы без ошибок возвращаем вещественный тип данных, иначе возвращаем ноль. Также вернем ноль, если значение `NaN`. Но если это не строка и не `NaN`, то просто возвращаем x. 
...
```
Этот участок кода позволяет нам запросить все числа.

В `month_cols` мы получаем список столбцов с результатами отгрузки. Например, `Апрель 2023`.

А `apply` позволяет нам в DataFrame применить функцию к колонке.
...
```
В итоге `later_months_from_prev` содержит месяцы, последующие после предыдущего, а `later_months_from_prev2` — после предпредыдущего. С помощью `month_number_to_name` мы превращаем номер месяца в слово, а тут `{prev_dt.year + ((prev_dt.month + i - 1) // 12)}` вычисляем сам год. С помощью `% 12` мы гарантируем, что наши номера месяцев будут в диапазоне от 1 до 12. Ну и само `i` мы тоже перебираем в диапазоне от 1 до 13. Если что, то мы ставим 13 там просто потому, что у нас `range` особенный — он не берет верхнюю границу диапазона, а берет последнее значение перед границей. 

Далее мы перебираем все наши аккаунты, сгруппировав DataFrame по колонке `Account`.
...
Вот и весь основной алгоритм. Тут главное было понять в самой задаче, как вычислять эти K1 и K2.


Ну дальше нам остается выгрузить список месяцев из таблицы пролонгации `df_prolong`. И уже перебирая их подставлять в нашу функцию вычисления. А там, если результат не пуст, то записываем в массив result.
```
# Список месяцев
...
print(final_df.head())
```
Для большего удобства я вручную сформировал одну таблицу `Общий отчет всего`, где все данные из всех 4-х файлов собраны в один файл, но по разным листам. 

# Инструкция «Открытие файла csv в Excel»

У многих возникают вопросы, а как открыть `csv` в `Excel`. Легко и просто! При открытии может произойти так, что данные будут в одной колонке. Но все данные разделены знаками-разделителями. Чаще всего `;` — это знак-разделитель, но могут быть запятые. Чтоб всё распределить нормально, мы нажимаем на `Данные` на панели вкладок, а потом находим `Текст по столбцам`. Дальше мы выбираем `с разделителем` -> `ОК`. А потом выбираем тип разделителя знака. Жмём `ОК`, а затем `Готово`. 

То же самое при копировании csv в Excel. Всё вставится в одну колонку, а дальше делай то же самое.

# Вывод

Выполняя эту задачу, мне удалось реализовать программу, которая будет сама вычислять статистику по пролонгации каждого менеджера. Такая программа и вправду поможет руководству выявлять неэффективных менеджеров. 

Спасибо, что зашел посмотреть данный код. Я надеюсь, что тебе было все ясно. Мои контакты можно увидеть на моем профиле!
